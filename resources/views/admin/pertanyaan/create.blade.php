    ...
    <x-slot name="header">
        <div>
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Tambah Pertanyaan untuk {{ $kuisioner->nama_kuisioner }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-10">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 space-y-6">
                <form action="{{ route('admin.kuisioner.pertanyaan.store', $kuisioner) }}" method="POST" class="space-y-4">
                    @csrf
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-slate-700">Teks Pertanyaan</label>
                        <textarea name="teks_pertanyaan" rows="4" class="w-full rounded-2xl border-slate-200 focus:ring-blue-500 focus:border-blue-500">{{ old('teks_pertanyaan') }}</textarea>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="tipe_jawaban" class="w-full rounded-2xl border-slate-200 focus:ring-blue-500 focus:border-blue-500">
                            <option value="likert" @selected(old('tipe_jawaban') === 'likert')>Skala Likert</option>
                            <option value="text" @selected(old('tipe_jawaban') === 'text')>Isian Teks</option>
                            <option value="dropdown" @selected(old('tipe_jawaban') === 'dropdown')>Dropdown</option>
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="opsi_dropdown[]" rows="3" placeholder="Gunakan koma untuk memisahkan opsi" class="w-full rounded-2xl border-slate-200 focus:ring-blue-500 focus:border-blue-500">{{ old('opsi_dropdown.0') }}</textarea>
                    </div>
