            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 space-y-6">
                <form action="{{ route('admin.dosen.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="nama" value="{{ old('nama') }}" class="w-full rounded-2xl border-slate-200 focus:ring-blue-500 focus:border-blue-500">
                        @error('nama') <p class="text-sm text-red-500">{{ $message }}</p> @enderror
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-slate-700">NIP</label>
                        <input type="text" name="nip" value="{{ old('nip') }}" class="w-full rounded-2xl border-slate-200 focus:ring-blue-500 focus:border-blue-500">
