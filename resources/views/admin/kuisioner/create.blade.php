<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Kuisioner</h2>
        </div>
    </x-slot>
    ...
    <div class="py-10">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 space-y-6">
                <form action="{{ route('admin.kuisioner.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-slate-700">Nama Kuisioner</label>
                        <input type="text" name="nama_kuisioner" value="{{ old('nama_kuisioner') }}" class="w-full rounded-2xl border-slate-200 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="deskripsi" rows="4" class="w-full rounded-2xl border-slate-200 focus:ring-blue-500 focus:border-blue-500">{{ old('deskripsi') }}</textarea>
